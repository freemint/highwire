...
int main(void)
{
	...
	fprintf(file,
	    ...
	    "<META NAME=\"copyright\" CONTENT=\"Copyright © Rainer Seitel, 2002\">\n"
	...
	for (i = 0; i < numberof(windows1252_to_BICS); i++) {
		if (windows1252_to_BICS[i] < 564)
			...
			printf("uni_bics: error windows1252_to_BICS[%d] = %d = $%X ò 564\n",
	...
	for (i = 0; i < numberof(Unicode_to_BICS); i++) {
		...
		while (Unicode_to_BICS[i]) {
			if (Unicode_to_BICS[i] < 564)
				...
				printf("uni_bics: error Unicode_to_BICS[%d] = %d = $%X ò 564\n",
