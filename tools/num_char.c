...
int main(void)
{
	...
	/* num_char needs a Speedo fonts GDOS, that is SpeedoGDOS or NVDI ò 3.
	...
	if ((gdostype != GDOS_FSM) && !(gdostype == -65536L && getcookie(0x46534D43L, NULL))) {
		fprintf(stderr, "num_char needs SpeedoGDOS or NVDI ò 3!");
		return EXIT_FAILURE;
	}
	...
	if (!getcookie(0x4E564449L, &nvdi) || !nvdi || nvdi->nvdi_version < 0x400) {
		printf("Unicode TrueType fonts need NVDI ò 4.  Not found.\n#\n");
